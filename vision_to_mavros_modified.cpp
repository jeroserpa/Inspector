...
int main(int argc, char** argv)
{
  ...
  tf::TransformListener tf_map_listener; //map to odometry transform listener (thats given by rviz)
  
  tf::TransformBroadcaster br; //broadcaster of a tf FCU

  tf::StampedTransform transform; //original transform from odometry frame to t265_link
  
  tf::StampedTransform transform_map_odom ,transform_map_link; //map to odometry transform and total transform map to t265_link 
  ...
  while (node.ok())
  {
    ...
    try
    {
      ...
      tf_listener.lookupTransform(target_frame_id, source_frame_id, now, transform); //get the odom to link transform
      tf_map_listener.lookupTransform("map", target_frame_id, now, transform_map_odom); //get the map to odom transform

      if(!multiply_stamped_tfs(transform_map_odom,transform, transform_map_link)){  ///get the map to link transform
          ROS_WARN("Multiplication not valid");
      } 
      ...
      if (last_tf_time < transform.stamp_)
      {
        ...
        static tf::Quaternion quat_cam, quat_cam_to_body_x, quat_cam_to_body_y, quat_cam_to_body_z, quat_rot_z, quat_body, quat_body_NED;
        ...
        position_orig = transform_map_link.getOrigin();
        ...
        quat_cam = transform_map_link.getRotation();
        ...
        quat_body_NED = quat_cam_to_body_x * quat_cam_to_body_y * quat_cam_to_body_z;
        quat_body.normalize();

        //trying to add tf in NED frame
        tf::Transform transform_NED;
        transform_NED.setRotation( quat_body );
        transform_NED.setOrigin( position_body );
       
        //br.sendTransform(tf::StampedTransform(transform_NED, ros::Time::now(),target_frame_id,"FCU"));
        br.sendTransform(tf::StampedTransform(transform_map_link, ros::Time::now(),"map","FCU"));
