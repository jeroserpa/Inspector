...
def simple_demo():
    ...
    c.takeoff(alt)
    while c.pose.position.z < alt-0.1 :
    	print(c.pose.position.z)
	rospy.sleep(1)
    d_x,d_y,d_z = raw_input('Enter x,y,z position.\n').split(",")
    print("move ")
    d_x = float(d_x)
    d_y = float(d_y)
    d_z = float(d_z)
    c.goto_xyz_rpy(d_x,d_y,d_z,0,0,0)
    while c.pose.position.z < d_x-0.1 or c.pose.position.z < d_y-0.1 or c.pose.position.z < d_z-0.1 : 
    	print(c.pose.position)
	
    sec = raw_input('waiting key to land.\n')
